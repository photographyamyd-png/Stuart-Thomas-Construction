...
import { rediRockServiceCallouts } from "@/data/archive/copy-baseline/redi-rock";
import type { ServiceDetail, ServiceSlug } from "@/data/archive/copy-baseline/services";
import { getAdjacentServices, getServiceBySlug } from "@/data/archive/copy-baseline/services";
import { ServiceCapabilitiesBand } from "@/components/stc/enterprise/service/ServiceCapabilitiesBand";
import { ServicePager } from "@/components/stc/enterprise/service/ServicePager";
import { ServiceRediRockCallout } from "@/components/stc/enterprise/service/ServiceRediRockCallout";
import { ServiceWorkShowcase } from "@/components/stc/enterprise/service/ServiceWorkShowcase";
...
export function BaselineEnterpriseServicePage({ service, rediRockInstallPhoto }: Props) {
    ...
    <>
        ...
        {isImmersiveHero ? (
          <>
            <div className="stc-svc-page__hero-nav">
              <Link className="stc-page-back" href={B}>
                ← Back to homepage
              </Link>
              <p className="stc-svc-page__breadcrumb">
                <Link href={`${B}/services`}>Services</Link> / {service.title}
              </p>
            </div>
        ...
        <div className="container">
          ...
          <ul className="stc-svc-page__related-pills">
            {service.relatedSlugs.map((rel) => {
              ...
              return (
                <li key={rel}>
                  <Link href={`${B}/services/${rel}`}>{r.title}</Link>
                </li>
              );
            })}
            {isRediRockServiceSlug(service.slug) && (
              <li>
                <Link href={`${B}/materials/redi-rock`}>Redi-Rock Installation</Link>
              </li>
            )}
          </ul>
          ...
          <ul className="stc-svc-page__related-pills">
            {areas.map((area) => (
              <li key={area.slug}>
                <Link href={`${B}/areas/${area.slug}`}>{area.name}</Link>
              </li>
            ))}
          </ul>
        </div>
      ...
      >
        ...
        <div className="turner-regional__copy">
          ...
          <Link href={`${B}/contact`} className="btn-green stack-cta cta-self-start">

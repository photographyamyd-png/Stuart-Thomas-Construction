import type { ServiceSlug } from "./services";
import { resolveImagePath as r } from "./image-catalog";

/** Canonical image paths — single source for service page assignments */
const img = {
  armourBoulderWallWaterfront: r("/images/20260508_110404.jpg"),
  armourRiverRockRetaining: r("/images/20260508_105516.jpg"),
  armourStoneStairs: r("/images/20260508_110106.jpg"),
  armourStoneStairsSlope: r("/images/20260508_110056.jpg"),
  excavationBuildingPad: r("/images/20260508_093859.jpg"),
  excavationGradedPad: r("/images/20260508_110558.jpg"),
  excavationGravelDrive: r("/images/20260508_105213.jpg"),
  excavationLongDrive: r("/images/20260508_110917.jpg"),
  firePitGravel: r("/images/20260508_102632.jpg"),
  homeShowcasePanorama: r("/images/20260508_102657.jpg"),
  gravelPadFinish: r("/images/20260508_105454.jpg"),
  landscapeEarthworkBerm: r("/images/20260508_105643.jpg"),
  landscapeFinishEstablish: r("/images/20260508_110636.jpg"),
  landscapeOutdoorLiving: r("/images/20260508_105741.jpg"),
  landscapePlantingBeds: r("/images/20260508_105457.jpg"),
  patioWaterfront: r("/images/20260508_110051.jpg"),
  pergolaRockGarden: r("/images/20260508_110330.jpg"),
  projectsCourtShed: r("/images/20260508_105445.jpg"),
  waterfrontWalkway: r("/images/20260508_105835.jpg"),
  waterfrontPatioElevated: r("/images/20260508_105812.jpg"),
  waterfrontWideView: r("/images/20260508_105828.jpg"),
  waterfrontCabana: r("/images/20260508_110010.jpg"),
  waterfrontPatioBeach: r("/images/20260508_110046.jpg"),
  rediRockLogo: "/images/partners/redi-rock-logo.png",
  sarjeantCoLogo: "/images/partners/sarjeant-co-logo.png",
  rediRockReferenceHero: "/images/redi-rock/reference/hero-waterfront-reference.jpg",
  snowRemovalLoaderHero: "/images/snow-removal/hero-storm-plow.png",
  snowRemovalIndustrialLot: "/images/snow-removal/commercial-lot-loader.png",
  snowRemovalPlowFleet: "/images/snow-removal/rural-road-clearing.png",
  rtLandscapingBackyard: r("/images/retouched/stc-tiny-township-landscaping.jpg"),
  rtLandscapingFlagstoneYard: r("/images/retouched/stc-040.jpg"),
  rtLandscapingStoneStairs: r("/images/retouched/stc-036.jpg"),
  rtLandscapingGardenPath: r("/images/retouched/stc-321.jpg"),
  rtLandscapingWoodedPath: r("/images/retouched/stc-001.jpg"),
  rtHardscapingFlagstoneWalk: r("/images/retouched/stc-006.jpg"),
  rtExcavationHitachiFleet: r("/images/retouched/stc-116.jpg"),
  rtExcavationDualMachines: r("/images/retouched/stc-120.jpg"),
  rtExcavationHitachiSitePrep: r("/images/retouched/stc-275.jpg"),
  rtExcavationHitachiWoods: r("/images/retouched/stc-228.jpg"),
  rtExcavationHitachiResidential: r("/images/retouched/stc-250.jpg"),
  rtExcavationHitachiAction: r("/images/retouched/stc-214.jpg"),
  rtExcavationBobcatGravel: r("/images/retouched/stc-090.jpg"),
  rtExcavationBobcatT190: r("/images/retouched/stc-058.jpg"),
  rtExcavationGravelPile: r("/images/retouched/stc-114.jpg"),
} as const;

export const media = {
  homeHeroVideoStartSec: 4,
  homeHero: img.waterfrontWalkway,
  homeHeroPoster: img.rtLandscapingFlagstoneYard,
  homeHeroVideo: "/video/home-hero.mp4",
  /** Drone yard reveal clip — landscaping & hardscaping service heroes */
  landscapingHeroVideo: "/video/landscaping-hero.mp4",
  hardscapingHeroVideo: "/video/landscaping-hero.mp4",
  integritySection: img.rtExcavationHitachiFleet,
  ctaBanner: img.rtLandscapingBackyard,
  homeShowcasePanorama: img.homeShowcasePanorama,
  landscapePlantingBeds: img.landscapePlantingBeds,
  landscapeFinishEstablish: img.landscapeFinishEstablish,
  aboutHero: img.rtExcavationDualMachines,
  projectsHero: img.rtExcavationHitachiWoods,
  contactHero: img.rtLandscapingBackyard,
  /** Header featured panel + baseline insight cards */
  featuredNavWaterfront: img.waterfrontWalkway,
  insightWaterfront: img.waterfrontWalkway,
  insightRediRock: img.rediRockReferenceHero,
  insightHardscaping: img.rtLandscapingFlagstoneYard,
  insightExcavation: img.rtExcavationHitachiFleet,
  serviceDefaults: {
    "armour-stone": img.armourBoulderWallWaterfront,
    "waterfront-stone-work": img.waterfrontWalkway,
    landscaping: img.rtLandscapingBackyard,
    hardscaping: img.rtLandscapingFlagstoneYard,
    excavation: img.rtExcavationHitachiFleet,
    "commercial-snow-removal": img.snowRemovalLoaderHero,
  } satisfies Record<ServiceSlug, string>,
  serviceCapabilityImages: {
    "armour-stone": [
      img.armourRiverRockRetaining,
      img.armourStoneStairs,
      img.armourBoulderWallWaterfront,
    ],
    "waterfront-stone-work": [
      img.waterfrontWalkway,
      img.waterfrontPatioBeach,
      img.waterfrontWideView,
    ],
    landscaping: [
      img.rtLandscapingWoodedPath,
      img.rtLandscapingFlagstoneYard,
      img.rtLandscapingGardenPath,
    ],
    hardscaping: [
      img.rtHardscapingFlagstoneWalk,
      img.rtLandscapingStoneStairs,
      img.rtLandscapingFlagstoneYard,
    ],
    excavation: [
      img.rtExcavationBobcatGravel,
      img.rtExcavationBobcatT190,
      img.rtExcavationHitachiSitePrep,
    ],
    "commercial-snow-removal": [
      img.snowRemovalIndustrialLot,
      img.snowRemovalPlowFleet,
      img.snowRemovalLoaderHero,
    ],
  } satisfies Record<ServiceSlug, readonly [string, string, string]>,
  featuredGalleryPaths: [
    img.waterfrontWalkway,
    img.rediRockReferenceHero,
    img.rtLandscapingFlagstoneYard,
    img.rtExcavationHitachiFleet,
    img.armourBoulderWallWaterfront,
    img.patioWaterfront,
  ],
  areaHeroes: {
    "tiny-township": img.rtLandscapingStoneStairs,
    "wasaga-beach": img.patioWaterfront,
    collingwood: img.rtLandscapingFlagstoneYard,
    midland: img.snowRemovalIndustrialLot,
    penetanguishene: img.snowRemovalLoaderHero,
  },
  rediRockLogo: img.rediRockLogo,
  sarjeantCoLogo: img.sarjeantCoLogo,
  rediRockReferenceHero: img.rediRockReferenceHero,
} as const;

export function getServiceHero(slug: ServiceSlug): string {
  return media.serviceDefaults[slug];
}

export function getServiceCapabilityImages(slug: ServiceSlug): string[] {
  return [...media.serviceCapabilityImages[slug]];
}

export type ServiceWorkShowcase = {
  eyebrow: string;
  headline: string;
  statement?: string;
  leadImage: string;
  leadAlt: string;
  supporting: { src: string; alt: string; caption: string }[];
};

export const serviceWorkShowcase: Partial<Record<ServiceSlug, ServiceWorkShowcase>> = {
  "armour-stone": {
    eyebrow: "On Site",
    headline: "Armour Stone In Practice",
    statement: "Armour stone retaining and stairs on Georgian Bay and Tiny Township properties.",
    leadImage: img.rtLandscapingStoneStairs,
    leadAlt: "Armour stone stairs and retaining wall in Tiny Township",
    supporting: [
      {
        src: img.armourRiverRockRetaining,
        alt: "Armour stone retaining wall on a South Georgian Bay property",
        caption: "Shoreline retention",
      },
      {
        src: img.armourStoneStairs,
        alt: "Finished armour stone stairs at a cottage property",
        caption: "Finished stair interface",
      },
    ],
  },
};

export function getServiceWorkShowcase(slug: ServiceSlug): ServiceWorkShowcase | null {
  return serviceWorkShowcase[slug] ?? null;
}
